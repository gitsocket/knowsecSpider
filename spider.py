...
def _initDB(dbFile):
    ...
    try:
        ...
        logging.cratical(dbFile+' 数据库连接错误')
        exit()
    if not exist:
        try:
...
class showProgress(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            logging.error('curses初始化失败，可能未安装curses库')
        maxFile=[0]*(self.deep+1)
        while True:
            links=list(self.QLinks.__dict__['queue'])
            deeps=[x[1] for x in links] #队列中每个URL此时的深度值 
            '''keys中元素是[deep值,次数]，
...
if __name__=='__main__':
    ...
    deep-=1
    urls=[] #防止抓取重复URL
    fileMD5=[] #保存文件的MD5值防止url不同但内容相同的文件重复下载
    mainHandler(threadNum,url,deep,key,'testself' in args)
